...
public class UserAuthenticationServiceImpl implements UserAuthenticationService {
    ...
    public User findByUserIdAndPassword(String userId, String password) throws UserNotFoundException {
    	try {
			User findByUserIdAndUserPassword = userAuthenticationRepository.findByUserIdAndUserPassword(userId, password);
				if (findByUserIdAndUserPassword!=null) {
					return findByUserIdAndUserPassword;
				} else {
    ...
    public boolean saveUser(User user) throws UserAlreadyExistsException {
    	...
    	try
    	{
    		User findById = userAuthenticationRepository.findById(user.getUserId()).get();
    		if (findById!=null) {
    	...
    	{
    		User userAdded = userAuthenticationRepository.save(user);
	    	if(userAdded!=null)
	    	{
	    		return  true;
	    	}
